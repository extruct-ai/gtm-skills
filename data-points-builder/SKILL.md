...
description: >
  Design enrichment columns that bridge research hypotheses to table enrichment.
  ...
  table-enrichment. Triggers on: "data points", "enrichment columns", "column
...
- After `market-problems-deep-research` has produced a hypothesis set
- Before `table-enrichment` — this skill designs the columns, that skill runs them
...
**Input:** Hypothesis set (from `market-problems-deep-research` or context file)
...
Generate the final list ready for `table-enrichment`:
...
1. Save `column_configs` as a Python variable or JSON for the user
2. Tell the user: "These configs are ready for `table-enrichment`. Run that skill with your table ID and these columns."
3. If the user wants to run immediately, hand off to `table-enrichment` workflow
