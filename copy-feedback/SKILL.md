...
```
email-generation → copy-feedback → run-instantly
```
...
| Prospect LinkedIn URL | strongly recommended | From `linkedin-finder` output |
| Hypothesis matched | recommended | From `segment-and-tier` output |
| Extruct people table data | optional | LinkedIn Data column from `linkedin-finder` |
...
If LinkedIn data is available from the Extruct people table (linkedin_data column), use it directly. Otherwise, run Perplexity queries.
...
- **Respect the voice rules.** All rewrites must stay within the voice constraints from `email-generation` / `cold-email`.
