...
The `columns` param takes an array of column UUIDs. Only cells in those columns will be queued. Use this when adding a new column to avoid re-running existing enrichments.
...
Empty body runs ALL pending cells across ALL columns and rows. Use only when uploading new rows that need all columns enriched.
