...
description: >
  Build and maintain a global company context file that all other GTM pipeline
  skills read from. Captures product info, voice rules, ICP, win cases, proof
...
One global context file per company. Every other GTM pipeline skill reads from this file for voice, value prop, ICP, win cases, proof points, and campaign learnings.
...
| Voice | Sender name, tone, language level, hard constraints, banned words, scope boundaries | Who emails come from, how they sound |
...
- Email-safe value prop (same value prop rewritten without any banned words from Voice section)
...
- Update voice rules or banned words
...
Use when importing campaign results from Instantly or manual tracking.

**Step 1:** Read campaign results (CSV, pasted data, or Instantly export).
...
- `cold-email` — reads Voice, What We Do, Proof Library, and Active Hypotheses to build prompt templates
...
- `market-problems-deep-research` — reads ICP and hypotheses for research scope
- `data-points-builder` — reads hypotheses for segmentation column design
- `segment-and-tier` — reads hypotheses for tiering logic
- `copy-feedback` — reads Voice rules to constrain rewrites
- `run-instantly` — reads DNC list for exclusions
